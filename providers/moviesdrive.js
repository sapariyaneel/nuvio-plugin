...
function getRegistry() {
  ...
  return fetch(DOMAINS_URL, { headers: { "User-Agent": "Mozilla/5.0" }, redirect: "follow" }).then((r) => r.json()).then((d) => {
    cachedRegistry = d || {};
...
function pixelDrainExtractor(link2) {
  return Promise.resolve().then(() => {
    ...
    return fetch(infoUrl, { headers: HEADERS, redirect: "follow" }).then((response) => response.json()).then((info) => {
      if (info && info.name) {
...
function hubStreamExtractor(url, referer) {
  return fetch(url, { headers: { ...HEADERS, Referer: referer }, redirect: "follow" }).then((response) => {
...
function hbLinksExtractor(url, referer) {
  return fetch(url, { headers: { ...HEADERS, Referer: referer }, redirect: "follow" }).then((response) => response.text()).then((data) => {
...
function resolveHubcloudSearchRecover(url) {
  try {
    ...
    return fetch(apiUrl, { headers: { ...HEADERS, Accept: "application/json" }, redirect: "follow" }).then((r) => r.json()).then((data) => data && Array.isArray(data.hits) && data.hits[0] && data.hits[0].url || null).catch(() => null);
  } catch (e) {
    return Promise.resolve(null);
  }
}
  ...
  return getCurrentDomain().then((currentDomain) => {
    ...
    if (isMovie) {
      ...
      const mdrivePromises = seasonPageUrls.map(
        (seasonPageUrl) => fetch(seasonPageUrl, { headers: HEADERS, redirect: "follow" }).then((r) => r.text()).then((episodeHtml) => extractEpisodeLinks(episodeHtml, episodePattern)).catch(() => [])
      );
