      ...
      return Promise.all(serverPromises).then(() => {
        ...
        const nuvioStreams = dedupedStreams.map(stream => ({
          ...
          title: formatStreamTitle({
            title: title,
            year: year,
            season: mediaType === 'tv' && seasonNum && episodeNum ? seasonNum : undefined,
            episode: mediaType === 'tv' && seasonNum && episodeNum ? episodeNum : undefined,
            rawText: `${stream.serverType || ''} ${stream.type || ''}`,
            url: stream.url,
