  ...
  try {
    ...
    const streams = await Promise.all(sources.filter(s => s && s.url).map(async (s) => {
      ...
      return {
        ...
        title: `Vidking ${s.quality || "Unknown"}`,
        name: "Vidking",
        size,
        headers: HEADERS,
        subtitles
