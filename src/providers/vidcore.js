...
const FALLBACK_EMBED_BASE = "https://vidcore.org";
...
async function scrapeMirror(apiBase, referer, mirrorId, tmdbId, mediaType, title) {
  try {
    ...
    if (!data || !Array.isArray(data.stream) || !data.stream.length) return null;
    return data.stream;
  } catch (e) {
    return null;
  }
...
async function getStreams(tmdbId, mediaType, season, episode) {
  try {
    ...
    const referer = isTv
      ? `${embedBase}/embed/tv/${numericTmdbId}/${season || 1}/${episode || 1}`
      : `${embedBase}/embed/movie/${numericTmdbId}`;
